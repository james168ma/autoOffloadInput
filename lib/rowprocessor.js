...
async function processRow(rowData, services, options) {
    ...
    const {
        ...
        rowNumber,
        CL_API_KEY,
        FORCE_CL_OVERWRITE = false,
        FORCE_CONFIDENCE_OVERWRITE = false,
        FORCE_GRADE_OVERWRITE = false,
    } = options;
    ...
    if (needsName || needsNumber || needsGrade) {
        if (['BOTH', 'PSA'].includes(WRITE_MODE)) {
            ...
            if (psaData) {
                ...
                console.log(`⚠️ PSA Data failed to load for missing fields. Marking RED.`);
                instructions.writePsaErrorColor = true;
                instructions.rowModified = true;
            }
        } else {
    ...
    if (['BOTH', 'CL', 'CONFIDENCE'].includes(WRITE_MODE)) {
        ...
        // For CONFIDENCE mode, always scrape. For CL/BOTH, check SKIP_CL_CHECK
        if (WRITE_MODE !== 'CONFIDENCE' && SKIP_CL_CHECK && currentVal && String(currentVal).trim() !== '') {
            shouldScrape = false;
        }

        if (shouldScrape) {
            console.log(`🔎 Checking CL Value...`);
            const result = await getCLValue(page, cert, lastScrapedValue, isSameCard, CL_API_KEY);
            console.log(`CL Value Result: ${JSON.stringify(result)}`);

            if (result !== null && result.raw > 0) {
                const { raw, higher, confidence, grade } = result;

                // Store confidence for all modes (BOTH, CL, CONFIDENCE)
                const hasCurrentConfidence = instructions.writeConfidence !== undefined;
                if (!hasCurrentConfidence || FORCE_CONFIDENCE_OVERWRITE) {
                    if (hasCurrentConfidence && FORCE_CONFIDENCE_OVERWRITE) {
                        console.log(`✏️ Overwriting CL confidence for ${cert}: ${confidence || 0}`);
                    }
                    instructions.writeConfidence = confidence || 0;
                }

                // Update State
                instructions.updatedLastScrapedValue = raw;

                // Handle CL Value (only for BOTH and CL modes, not CONFIDENCE)
                if (['BOTH', 'CL'].includes(WRITE_MODE)) {
                    ...
                    if (!hasCurrentVal || FORCE_CL_OVERWRITE) {
                        if (hasCurrentVal && FORCE_CL_OVERWRITE) {
                            const cleanCurrent = parseFloat(String(currentVal).replace(/[^0-9.]/g, ''));
                            console.log(
                                `✏️ Overwriting CL value for ${cert}: ${cleanCurrent} -> ${newValToWrite}`
                            );
                        }
                        // Write it (optionally overwrite existing)
                ...
                // If PSA is skipped (CL-only) and grade is missing, use CL API grade
                if (WRITE_MODE === 'CL') {
                    const needsGrade = !currentGrade || String(currentGrade).trim() === '';
                    if (grade !== null && grade !== undefined && (needsGrade || FORCE_GRADE_OVERWRITE)) {
                        if (!needsGrade && FORCE_GRADE_OVERWRITE) {
                            console.log(
                                `✏️ Overwriting grade for ${cert}: ${currentGrade} -> ${grade}`
                            );
                        }
                        instructions.writeGrade = grade;
                        instructions.rowModified = true;
                    }
                }

                // Confidence is written regardless of mode (handled in index.js)
                instructions.rowModified = true;
            } else {
                // Result is null (error/timeout) OR raw is 0 (value not found)
                const hasCurrentVal = currentVal && String(currentVal).trim() !== '';

                if (!hasCurrentVal && WRITE_MODE !== 'CONFIDENCE') {
                    console.log(`⚠️ CL Value failed to load/is 0 for empty cell. Marking RED.`);
                    instructions.writeValue = 'No comps';
                    instructions.writeErrorColor = true;
                    instructions.rowModified = true;
                }
            }
        } else {
